...
A seguir, estão todas as palavras reservadas que estão no escopo do nosso compilador, junto com a explicação do que cada uma faz.

| Palavras Reservadas | Token | Descrição |
| :--- | :--- | :---: |
| and | TOK_LOGIC_AND | Sinônimo para o operador lógico de conjunção && |
...
| cout | TOK_COUT | **(Token Especial)** Objeto utilizado para impressão de dados na saída padrão |
...
| not | TOK_LOGIC_NOT | Sinônimo para o operador lógico de negação ! |
| nullptr | TOK_NULLPTR | Representa de forma segura um valor de ponteiro nulo |
| or | TOK_LOGIC_OR | Sinônimo para o operador lógico de disjunção // |
...
| std | TOK_STD | Identificador do namespace padrão utilizado para fluxos de I/O |
...
| void | TOK_VOID | Indica que uma função não retorna valor |
...
- **std:** Prefixo do namespace padrão para streams de entrada e saída.
...
- **and:** Alias para o operador lógico && (`TOK_LOGIC_AND`).
- **or:** Alias para o operador lógico || (`TOK_LOGIC_OR`).
- **not:** Alias para o operador lógico ! (`TOK_LOGIC_NOT`).
